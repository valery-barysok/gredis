...
func (client *Client) LPush(key string, value string, values ...string) (int, error) {
	item, err := client.Do(LPushCommand, toBulkArray(values, key, value)...)
	if err != nil {
		return 0, err
	}
...
func (client *Client) RPush(key string, value string, values ...string) (int, error) {
	item, err := client.Do(RPushCommand, toBulkArray(values, key, value)...)
	if err != nil {
		return 0, err
	}
...
func (client *Client) LPop(key string) ([]byte, error) {
	item, err := client.Do(LPopCommand, []byte(key))
	if err != nil {
		return nil, err
	}
...
func (client *Client) RPop(key string) ([]byte, error) {
	item, err := client.Do(RPopCommand, []byte(key))
	if err != nil {
		return nil, err
	}
...
func (client *Client) LLen(key string) (int, error) {
	item, err := client.Do(LLenCommand, []byte(key))
	if err != nil {
		return 0, err
	}
...
func (client *Client) LInsert(key string, before bool, pivot string, value string) (int, error) {
	...
	item, err := client.Do(LInsertCommand, []byte(key), place, []byte(pivot), []byte(value))
	if err != nil {
		return 0, err
	}
...
func (client *Client) LIndex(key string, index int) ([]byte, error) {
	item, err := client.Do(LIndexCommand, []byte(key), []byte(strconv.Itoa(index)))
	if err != nil {
		return nil, err
	}
...
func (client *Client) LRange(key string, start int, stop int) ([][]byte, error) {
	item, err := client.Do(LRangeCommand, []byte(key), []byte(strconv.Itoa(start)), []byte(strconv.Itoa(stop)))
	if err != nil {
		return nil, err
	}
